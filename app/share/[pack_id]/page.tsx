...
export async function generateMetadata(
  ...
  return {
    title: data ? `${data.name} — USA Wrap Co` : 'Photo Pack — USA Wrap Co',
    description: (data?.description as string | null) ?? 'Photo pack from USA Wrap Co',
  }
}
...
export default async function SharePackPage({ params }: { params: { pack_id: string } }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d0f14', color: '#e8eaed', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      {/* Header */}
      <div style={{ background: '#13151c', borderBottom: '1px solid #1a1d27', padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 16 }}>
        <div>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 900, color: '#e8eaed', textTransform: 'uppercase', letterSpacing: '0.03em', lineHeight: 1, fontFamily: '"Barlow Condensed", sans-serif' }}>
            {typedPack.name}
          </h1>
        ...
        <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
          <div style={{ fontSize: 11, color: '#5a6080', fontFamily: '"JetBrains Mono", monospace' }}>{files.length} files</div>
          <div style={{ fontSize: 11, color: '#5a6080', marginTop: 2, fontFamily: '"JetBrains Mono", monospace' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
        {files.length === 0 ? (
          ...
          <>
            ...
            {otherFiles.length > 0 && (
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 700, color: '#9299b5', marginBottom: 12, textTransform: 'uppercase', fontFamily: '"Barlow Condensed", sans-serif' }}>Documents &amp; Files</h2>
