...
const STATUS_COLOR: Record<string, string> = {
  moving: '#22c07a', active: '#22c07a', parked: '#9299b5',
  maintenance: '#f59e0b', inactive: '#5a6080',
}
const STATUS_LABEL: Record<string, string> = {
  moving: 'MOVING', active: 'ACTIVE', parked: 'PARKED',
  maintenance: 'SERVICE', inactive: 'OFFLINE',
}

export default function DealerFleetClient({ entityId }: { entityId: string }) {
  const [vehicles, setVehicles] = useState<FleetVehicle[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/fleet-map/vehicles')
      .then(r => r.ok ? r.json() : [])
      .then((all: FleetVehicle[]) => setVehicles(all.filter(v => v.name !== null)))
      .catch(() => {})
  ...
  if (loading) return (
    <div style={{ padding: 20, textAlign: 'center', color: C.text3, paddingTop: 60 }}>Loading fleet data…</div>
  )
  ...
  return (
    <div style={{ padding: '16px 16px 20px' }}>
      ...
      <div style={{ fontSize: 12, color: C.text3, marginBottom: 20 }}>{vehicles.length} vehicle{vehicles.length !== 1 ? 's' : ''} tracked</div>

      <div style={{
        background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.25)',
        borderRadius: 12, padding: '16px 20px', marginBottom: 20, textAlign: 'center',
      }}>
        <div style={{ fontSize: 10, color: '#8b5cf6', fontWeight: 700, letterSpacing: '0.1em', marginBottom: 6 }}>
          TODAY&rsquo;S TOTAL IMPRESSIONS
        </div>
            ...
            <div key={v.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 9, fontWeight: 700, padding: '3px 7px', borderRadius: 4, background: statusColor + '20', color: statusColor, letterSpacing: '0.06em' }}>
                ...
                {v.wrap_description && <div style={{ fontSize: 11, color: C.text2, marginBottom: 6 }}>{v.wrap_description}</div>}
                {serviceOverdue ? (
