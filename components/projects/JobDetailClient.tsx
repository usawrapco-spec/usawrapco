...
export default function JobDetailClient({
  ...
  const [surveyData, setSurveyData] = useState<Record<string, any>>(
    (((project.form_data ?? {}) as Record<string, any>).survey ?? {}) as Record<string, any>
  )
...
const _headingFont = 'Barlow Condensed, sans-serif'
const _monoFont    = 'JetBrains Mono, monospace'
...
function SurveyTab({
  ...
  async function selectModel(model: string) {
    ...
    const snapshot: Record<string, any> = { ...surveyData, vehicleModel: model, vehicleSqft: sqft }
  ...
  function selectScope(scope: string) {
    ...
    onFieldChange('vehicleSqft', sqft)
    const snapshot: Record<string, any> = { ...surveyData, wrapScope: scope, vehicleSqft: sqft }
  ...
  const sLbl: React.CSSProperties = { display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4, fontFamily: _headingFont }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700, fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text1)' }}>
      ...
      {/* ── VEHICLE INFO ─────────────────────────────────────────────────────── */}
      <div style={sCard}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={ddStyle}>
                {filteredModels.map(m => (
                  <div key={m.model} style={{ ...optSt, display: 'flex', justifyContent: 'space-between' }}
                    ...
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <span>{m.model}</span>
                    {m.sqft && <span style={{ fontSize: 10, color: 'var(--text3)', fontFamily: _monoFont }}>{m.sqft}sqft</span>}
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['Front', 'Driver Side', 'Passenger Side', 'Rear', 'Damage'].map(angle => (
              <label key={angle} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '10px 14px', borderRadius: 8, border: '1px dashed var(--surface2)', cursor: canWrite ? 'pointer' : 'default', background: 'var(--bg)', minWidth: 80, color: photoAngles.includes(angle) ? 'var(--green)' : 'var(--text3)' }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, fontFamily: _headingFont }}>{angle}</span>
                {canWrite && (
      ...
      {/* ── WRAP SCOPE ──────────────────────────────────────────────────────── */}
      <div style={sCard}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--purple)', fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {sqftPreview !== null && vehicleEntry && (
          <div style={{ display: 'flex', gap: 20, marginBottom: 14, padding: '10px 14px', borderRadius: 8, background: 'rgba(34,192,122,0.06)', border: '1px solid rgba(34,192,122,0.15)' }}>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text3)', fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Est. Sqft</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--green)', fontFamily: _monoFont }}>{sqftPreview}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text3)', fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Full Wrap Sqft</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)', fontFamily: _monoFont }}>{vehicleEntry.sqft}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text3)', fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Base Rate</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)', fontFamily: _monoFont }}>${vehicleEntry.basePrice}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text3)', fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Install Hrs</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)', fontFamily: _monoFont }}>{vehicleEntry.installHours}h</div>
            </div>
          </div>
        )}
      ...
      {/* ── SURFACE CONDITION ────────────────────────────────────────────────── */}
      <div style={sCard}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--amber)', fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {/* ── CUSTOMER SIGN-OFF ────────────────────────────────────────────────── */}
      <div style={sCard}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--green)', fontFamily: _headingFont, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
