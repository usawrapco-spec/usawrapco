...
function CustomerSearchModal({
  ...
  async function handleCreate() {
    ...
    const { data, error } = await supabase
      .from('customers')
      .insert({ org_id: orgId, name: newName.trim(), company_name: newCompany.trim() || null, phone: newPhone.trim() || null, email: newEmail.trim() || null })
      .select('id, name, email, phone, company_name, lifetime_spend')
      .single()
    setCreating(false)
        ...
        <div style={{ padding: '16px 20px 12px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          {!createMode && (
            <div style={{ position: 'relative' }}>
              ...
              />
              {searching && (
                <div style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 14, height: 14, border: '2px solid var(--accent)', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} />
              )}
            </div>
          )}
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 12px' }}>
          {!createMode ? (
            <>
              ...
              {results.map(c => (
                <button
                  key={c.id}
                  onClick={() => onSelect(c)}
                  style={{ ...rowStyle, width: '100%', textAlign: 'left', background: 'none' }}
            ...
            <div style={{ padding: '8px 4px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              {(['Name *', 'Company', 'Phone', 'Email'] as const).map((label, i) => {
                const vals = [newName, newCompany, newPhone, newEmail]
                const setters = [setNewName, setNewCompany, setNewPhone, setNewEmail]
                return (
                  <div key={label}>
                    ...
                    <input value={vals[i]} onChange={e => setters[i](e.target.value)} style={inputStyle} />
                  </div>
                )
              })}
            </div>
