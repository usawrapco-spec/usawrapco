...
export default function EstimateDetailClient({ profile, estimate, employees, customers, isNew }: Props) {
  ...
  const [linkedCustomer, setLinkedCustomer] = useState<CustomerRow | null>(
    est.customer ? { id: est.customer.id, name: est.customer.name, email: est.customer.email ?? null, phone: null, company_name: null, lifetime_spend: null } : null
  )
  ...
  useEffect(() => {
    ...
    loadTemplates().catch((error) => { console.error(error); })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])
            ...
            {moreMenuOpen && (
              <div style={{
                ...
                <MenuButton icon={<FileDown size={13} style={{ color: 'var(--green)' }} />} label="Quote" onClick={() => {
                  ...
                  const url = URL.createObjectURL(blob)
                  const a = document.createElement('a'); a.href = url; a.download = `USA-Wrap-Co-Quote-EST-${est.estimate_number}${customerSlug ? `-${customerSlug}` : ''}.csv`
          ...
          <div style={{ ...sectionPad, borderRight: '1px solid var(--border)' }}>
            ...
            {linkedCustomer ? (
              <div>
                ...
                {linkedCustomer.company_name && (
                  <div style={{ fontSize: 11, color: 'var(--cyan)', marginBottom: 2 }}>{linkedCustomer.company_name}</div>
                )}
                {linkedCustomer.phone && (
                  <div style={{ fontSize: 12, color: 'var(--text2)', fontFamily: 'JetBrains Mono, monospace' }}>{linkedCustomer.phone}</div>
                )}
                {linkedCustomer.email && (
                  <div style={{ fontSize: 12, color: 'var(--text2)' }}>{linkedCustomer.email}</div>
                )}
                {(vehicleYear || vehicleMake || vehicleModel) && (
            ...
            <CustomerSearchModal
              ...
              onSelect={async (c) => {
                ...
                if (estimateId && !estimateId.startsWith('new-')) {
                  await supabase.from('estimates').update({ customer_id: c.id }).eq('id', estimateId)
                }
              }}
            />
          </div>
