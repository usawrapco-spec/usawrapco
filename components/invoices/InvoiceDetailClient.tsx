...
import {
  ...
  Package, StickyNote, Save, ClipboardCheck, Car, Camera, Layers, PenLine, ChevronDown,
} from 'lucide-react'
...
type InvTab = 'items' | 'tasks' | 'notes' | 'survey' | 'care'
...
// ─── Invoice Predefined Tasks ────────────────────────────────────────────────

const INV_TASKS: { key: string; label: string; required: boolean }[] = [
  { key: 'invoice_sent',        label: 'Send invoice to customer',                        required: true },
  { key: 'deposit_recorded',    label: 'Record deposit / down payment',                   required: true },
  { key: 'materials_confirmed', label: 'Confirm all materials & install costs are accurate', required: true },
  { key: 'job_complete',        label: 'Mark job as complete in pipeline',                required: true },
  { key: 'balance_collected',   label: 'Collect balance due on vehicle pickup',           required: true },
  { key: 'receipt_sent',        label: 'Send payment receipt to customer',                required: false },
  { key: 'google_review',       label: 'Request Google review from customer',             required: false },
  { key: 'job_closed',          label: 'Close job in system (mark done)',                 required: true },
  { key: 'warranty_info',       label: 'Send warranty & care instructions to customer',   required: false },
]

function InvTasksPanel({ tasks, onChange }: { tasks: Record<string, boolean>; onChange: (t: Record<string, boolean>) => void }) {
  const done = INV_TASKS.filter(t => tasks[t.key]).length
  const requiredDone = INV_TASKS.filter(t => t.required && tasks[t.key]).length
  const requiredTotal = INV_TASKS.filter(t => t.required).length

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: 'Barlow Condensed, sans-serif' }}>Invoice Tasks</div>
        <span style={{ fontSize: 11, color: requiredDone === requiredTotal ? 'var(--green)' : 'var(--amber)', fontWeight: 700 }}>
          {requiredDone}/{requiredTotal} required · {done}/{INV_TASKS.length} total
        </span>
      </div>
      <div style={{ height: 4, background: 'var(--surface2)', borderRadius: 2, marginBottom: 16, overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${(done / INV_TASKS.length) * 100}%`, background: done === INV_TASKS.length ? 'var(--green)' : 'var(--accent)', borderRadius: 2, transition: 'width 0.3s ease' }} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {INV_TASKS.map(task => (
          <label key={task.key} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '9px 12px', borderRadius: 8, background: tasks[task.key] ? 'rgba(34,192,122,0.06)' : 'var(--surface2)', border: `1px solid ${tasks[task.key] ? 'rgba(34,192,122,0.25)' : 'transparent'}`, transition: 'all 0.15s' }}>
            <input type="checkbox" checked={!!tasks[task.key]} onChange={() => onChange({ ...tasks, [task.key]: !tasks[task.key] })} style={{ width: 15, height: 15, accentColor: 'var(--green)', cursor: 'pointer', flexShrink: 0 }} />
            <span style={{ fontSize: 13, fontWeight: 600, color: tasks[task.key] ? 'var(--green)' : 'var(--text2)', flex: 1 }}>{task.label}</span>
            {task.required && !tasks[task.key] && <span style={{ fontSize: 9, fontWeight: 700, color: 'var(--amber)', textTransform: 'uppercase', letterSpacing: '0.06em', padding: '2px 6px', borderRadius: 4, background: 'rgba(245,158,11,0.12)', flexShrink: 0 }}>Required</span>}
...
export default function InvoiceDetailClient({ profile, invoice, lineItems = [], payments = [], isDemo, invoiceId, team = [], financingStatus = null, payLinkToken = null }: Props) {
  ...
  const [invTasks, setInvTasks] = useState<Record<string, boolean>>(
    (inv.form_data?.tasks as Record<string, boolean>) || {}
  )
  ...
  async function handleSaveLineItems(updatedItems: LineItem[]) {
    if (isDemo) return
    for (const li of updatedItems) {
      if (li.id.startsWith('new-')) {
        const { id: _id, ...rest } = li
        const { data } = await supabase.from('line_items').insert({ ...rest, org_id: inv.org_id }).select().single()
        if (data) {
          setLineItemsList(prev => prev.map(x => x.id === li.id ? { ...li, id: data.id } : x))
        }
      } else {
    ...
    const newSubtotal = updatedItems.reduce((s, li) => s + li.total_price, 0)
    const newTax = (newSubtotal - discount) * taxRate
    const newTotal = newSubtotal - discount + newTax
    await supabase.from('invoices').update({
      subtotal: newSubtotal, tax_amount: newTax, total: newTotal,
  ...
  async function handleRemoveLineItem(itemId: string) {
    ...
    const updated = lineItemsList.filter(x => x.id !== itemId)
    setLineItemsList(updated)
    await handleSaveLineItems(updated)
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => router.push('/invoices')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 12px', color: 'var(--text2)', fontSize: 13, cursor: 'pointer', minHeight: 44, justifyContent: 'center' }}>
            <ArrowLeft size={14} /> <span>Back</span>
          </button>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 900, fontFamily: invHeadingFont, color: 'var(--text1)', margin: 0, lineHeight: 1, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              <span style={{ color: 'var(--text3)', fontWeight: 600 }}>INV</span>{' '}
              <span style={{ fontFamily: invMonoFont }}>#{inv.invoice_number}</span>
            </h1>
            {inv.sales_order && (
              <button onClick={() => router.push(`/sales-orders/${inv.so_id}`)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontSize: 12, color: 'var(--text3)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                <ArrowLeft size={10} />
                <span style={{ fontFamily: invMonoFont }}>SO #{inv.sales_order.so_number}</span>
              </button>
            )}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {canWrite && status === 'draft' && (
            <button onClick={handleSendInvoice} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 14px', color: 'var(--text1)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: invHeadingFont, letterSpacing: '0.03em' }}>
              <Send size={14} /> Send Invoice
            </button>
          )}
          <button onClick={handleExportPdf} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 14px', color: 'var(--text1)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: invHeadingFont, letterSpacing: '0.03em' }}>
            <Download size={14} /> Download PDF
          </button>
          {canWrite && (
            <button onClick={handleSave} disabled={saving} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--green)', border: 'none', borderRadius: 12, padding: '9px 18px', color: '#fff', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.6 : 1, fontFamily: invHeadingFont, letterSpacing: '0.03em', boxShadow: '0 2px 12px rgba(34,192,122,0.25)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
          ...
          <div style={{ padding: '16px 20px', borderRight: '1px solid var(--card-border)' }}>
            ...
            {linkedCustomer ? (
              <div>
                ...
                {linkedCustomer.phone && <div style={{ fontSize: 12, color: 'var(--text2)', fontFamily: invMonoFont }}>{linkedCustomer.phone}</div>}
                {linkedCustomer.email && <div style={{ fontSize: 11, color: 'var(--text2)' }}>{linkedCustomer.email}</div>}
                {(vehicleYear || vehicleMake || vehicleModel) && (
                  <div style={{ fontSize: 11, color: 'var(--cyan)', marginTop: 4, fontFamily: invMonoFont }}>
          ...
          <div style={{ padding: '16px 20px', borderRight: '1px solid var(--card-border)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {(['draft','sent','open','partial','paid','overdue','void'] as const).map(s => {
                const c = STATUS_CONFIG[s]
                return (
                  <button key={s} onClick={() => canWrite && setStatus(s)} disabled={!canWrite || status === s}
                    style={{ padding: '3px 8px', borderRadius: 6, border: `1px solid ${status === s ? c.color : 'var(--border)'}`, background: status === s ? c.bg : 'transparent', color: status === s ? c.color : 'var(--text3)', fontSize: 10, fontWeight: 700, cursor: canWrite && status !== s ? 'pointer' : 'default', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    {c.label}
                  </button>
                )
              })}
          ...
          <div style={{ padding: '16px 20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div>
                <label style={{ fontSize: 11, color: 'var(--text3)' }}>Invoice Date</label>
                <input value={fmtDate(inv.invoice_date)} className="field" disabled style={{ padding: '4px 8px', fontSize: 12, opacity: 0.7 }} />
              </div>
              <div>
                <label style={{ fontSize: 11, color: status === 'overdue' ? 'var(--red)' : 'var(--text3)' }}>Due Date</label>
                <input value={fmtDate(inv.due_date)} className="field" disabled style={{ padding: '4px 8px', fontSize: 12, opacity: 0.7, color: status === 'overdue' ? 'var(--red)' : undefined }} />
              </div>
            </div>
          </div>
        </div>
        ...
        {showAllInfo && (
          <div style={{ borderTop: '1px solid var(--card-border)', padding: '16px 20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
              ...
              <div><label className="field-label">Make</label><input value={vehicleMake} onChange={e => setVehicleMake(e.target.value)} onBlur={e => handleSaveVehicle(vehicleYear, e.target.value, vehicleModel)} className="field" disabled={!canWrite} placeholder="Ford" /></div>
              <div><label className="field-label">Model</label><input value={vehicleModel} onChange={e => setVehicleModel(e.target.value)} onBlur={e => handleSaveVehicle(vehicleYear, vehicleMake, e.target.value)} className="field" disabled={!canWrite} placeholder="Transit 350" /></div>
              <div><label className="field-label">Color</label><input value={vehicleColor} onChange={e => setVehicleColor(e.target.value)} onBlur={e => handleSaveVehicle(vehicleYear, vehicleMake, vehicleModel, vehicleVin, e.target.value)} className="field" disabled={!canWrite} placeholder="White" /></div>
              <div style={{ gridColumn: 'span 3' }}><label className="field-label">VIN</label><input value={vehicleVin} onChange={e => setVehicleVin(e.target.value)} onBlur={e => handleSaveVehicle(vehicleYear, vehicleMake, vehicleModel, e.target.value, vehicleColor)} className="field" disabled={!canWrite} placeholder="1FTFW1E8..." /></div>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        <div style={{ padding: '12px 16px', borderRight: '1px solid var(--border)' }}>
          <div style={{ fontSize: 9, fontWeight: 800, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: invHeadingFont, marginBottom: 4 }}>Revenue</div>
          <div style={{ fontFamily: invMonoFont, fontVariantNumeric: 'tabular-nums', fontSize: 16, fontWeight: 700, color: 'var(--text1)' }}>{fmtCurrency(subtotal)}</div>
        </div>
        <div style={{ padding: '12px 16px', borderRight: '1px solid var(--border)' }}>
          <div style={{ fontSize: 9, fontWeight: 800, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: invHeadingFont, marginBottom: 4 }}>Total COGS</div>
          <div style={{ fontFamily: invMonoFont, fontVariantNumeric: 'tabular-nums', fontSize: 16, fontWeight: 700, color: 'var(--text1)' }}>{fmtCurrency(totalCOGS)}</div>
        </div>
        <div style={{ padding: '12px 16px', borderRight: '1px solid var(--border)' }}>
          <div style={{ fontSize: 9, fontWeight: 800, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: invHeadingFont, marginBottom: 4 }}>Gross Profit</div>
          <div style={{ fontFamily: invMonoFont, fontVariantNumeric: 'tabular-nums', fontSize: 16, fontWeight: 700, color: totalGP >= 0 ? 'var(--green)' : 'var(--red)' }}>{fmtCurrency(totalGP)}</div>
        </div>
        <div style={{ padding: '12px 16px' }}>
          <div style={{ fontSize: 9, fontWeight: 800, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: invHeadingFont, marginBottom: 4 }}>GPM</div>
          <div style={{ fontFamily: invMonoFont, fontVariantNumeric: 'tabular-nums', fontSize: 22, fontWeight: 900, color: overallGPM >= 73 ? 'var(--green)' : overallGPM >= 65 ? 'var(--amber)' : 'var(--red)' }}>{overallGPM.toFixed(1)}%</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', gap: 0, borderBottom: '2px solid var(--border)', overflowX: 'auto' }}>
              {([
                { key: 'items'  as InvTab, label: 'Items', count: lineItemsList.length },
                { key: 'tasks'  as InvTab, label: 'Tasks' },
                { key: 'notes'  as InvTab, label: 'Notes' },
                { key: 'survey' as InvTab, label: 'Survey' },
                { key: 'care'   as InvTab, label: 'Wash & Care' },
              ]).map(t => (
                <button
                  key={t.key}
                  ...
                  style={{
                    ...
                    fontFamily: invHeadingFont, textTransform: 'uppercase',
                ...
                >
                  ...
                  {(t as any).count !== undefined && (
                    <span style={{ fontFamily: invMonoFont, fontSize: 11, fontWeight: 700, background: activeTab === t.key ? 'rgba(79,127,255,0.2)' : 'rgba(90,96,128,0.15)', color: activeTab === t.key ? 'var(--accent)' : 'var(--text3)', padding: '1px 7px', borderRadius: 10 }}>
                      {(t as any).count}
                    </span>
                  )}
                </button>
                ...
                <div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          const newItem: LineItem = {
                            id: `new-${Date.now()}`, parent_type: 'invoice', parent_id: invoiceId,
                  ...
                  {lineItemsList.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: 32, color: 'var(--text3)', fontSize: 13 }}>
                      No line items on this invoice. Click &quot;Add Item&quot; to get started.
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {lineItemsList.map((li, idx) => (
                        <InvLineItemCard
                          ...
                          canWrite={canWrite}
                          onChange={(updated) => {
                            const newList = lineItemsList.map(x => x.id === li.id ? updated : x)
                            setLineItemsList(newList)
                          }}
                          onRemove={() => handleRemoveLineItem(li.id)}
                          onSave={() => handleSaveLineItems(lineItemsList)}
                        />
                      ))}
                    </div>
                  )}

                  {/* Totals summary below line items */}
                  {lineItemsList.length > 0 && (
              ...
              {activeTab === 'tasks' && (
                <InvTasksPanel
                  ...
                  onChange={async (updated) => {
                    setInvTasks(updated)
                    if (!isDemo) {
                      await supabase.from('invoices').update({ form_data: { ...inv.form_data, tasks: updated } }).eq('id', invoiceId)
                    }
                  }}
                />
              )}
                  ...
                  {showNotes && (
                    <>
                      ...
                      <textarea
                        ...
                        rows={4}
                        style={{ width: '100%', padding: '10px 12px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text1)', fontSize: 13, resize: 'vertical', outline: 'none', fontFamily: 'inherit' }}
                      />
                    </>
                  )}
                </>
          ...
          {canWrite && status !== 'void' && balanceDue > 0 && (
            <div className="card">
              <div className="section-label" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <CreditCard size={13} style={{ color: 'var(--green)' }} /> Record Payment
              </div>
              {showPaymentForm ? (
...
function InvLineItemCard({
  item, index, isExpanded, onToggle, canWrite, onChange, onRemove, onSave,
}: {
  item: LineItem; index: number; isExpanded: boolean; onToggle: () => void
  canWrite: boolean; onChange: (item: LineItem) => void; onRemove: () => void; onSave: () => void
}) {
    ...
    <div style={{
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 11, color: 'var(--text3)', fontFamily: invMonoFont }}>
            {index + 1}.
          </span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <ChevronDown size={14} style={{ color: 'var(--text3)', transform: isExpanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
        </div>
      </div>
      ...
      {isExpanded && (
        <div style={{ padding: '0 14px 14px', borderTop: '1px solid var(--border)' }}>
          ...
          <div style={{ marginTop: 10 }}>
            {!showDescription && (
              <button onClick={() => setShowDescription(true)} style={{ background: 'transparent', border: 'none', color: 'var(--accent)', fontSize: 12, fontWeight: 600, cursor: 'pointer', padding: '4px 0' }}>
                + Add Description
              </button>
            )}
            {showDescription && (
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              <div>
                ...
                <input value={(specs.vehicleYear as string) || ''} onChange={e => updateSpec('vehicleYear', e.target.value)} className="field" disabled={!canWrite} placeholder="2024" />
              </div>
              <div>
                <label className="field-label">Make</label>
                <input value={(specs.vehicleMake as string) || ''} onChange={e => updateSpec('vehicleMake', e.target.value)} className="field" disabled={!canWrite} placeholder="Ford" />
              </div>
              <div>
                <label className="field-label">Model</label>
                <input value={(specs.vehicleModel as string) || ''} onChange={e => updateSpec('vehicleModel', e.target.value)} className="field" disabled={!canWrite} placeholder="F-150" />
              </div>
              <div>
                <label className="field-label">Color</label>
                <input value={(specs.vehicleColor as string) || ''} onChange={e => updateSpec('vehicleColor', e.target.value)} className="field" disabled={!canWrite} placeholder="White" />
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              <Layers size={13} style={{ color: 'var(--purple, #8b5cf6)' }} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
              <div>
                ...
                <input value={(specs.wrapType as string) || ''} onChange={e => updateSpec('wrapType', e.target.value)} className="field" disabled={!canWrite} />
              </div>
              <div>
                <label className="field-label">Vinyl Type</label>
                <input value={(specs.vinylType as string) || ''} onChange={e => updateSpec('vinylType', e.target.value)} className="field" disabled={!canWrite} />
              </div>
              <div>
                <label className="field-label">Laminate</label>
                <input value={(specs.laminate as string) || ''} onChange={e => updateSpec('laminate', e.target.value)} className="field" disabled={!canWrite} />
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              <div>
                ...
                <input type="number" value={(specs.vinylArea as number) || ''} onChange={e => updateSpec('vinylArea', Number(e.target.value))} className="field mono" disabled={!canWrite} />
              </div>
              <div>
                <label className="field-label">Material Cost</label>
                <input type="number" value={(specs.materialCost as number) || ''} onChange={e => updateSpec('materialCost', Number(e.target.value))} className="field mono" disabled={!canWrite} />
              </div>
              <div>
                <label className="field-label">Labor Cost</label>
                <input type="number" value={(specs.laborCost as number) || ''} onChange={e => updateSpec('laborCost', Number(e.target.value))} className="field mono" disabled={!canWrite} />
              </div>
              <div>
                <label className="field-label">Complexity (1-5)</label>
                <input type="number" value={(specs.complexity as number) || ''} onChange={e => updateSpec('complexity', Number(e.target.value))} className="field mono" disabled={!canWrite} min={1} max={5} />
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: 14, display: 'flex', justifyContent: 'space-between' }}>
            {canWrite && (
              <button onClick={onRemove} className="btn-ghost btn-xs" style={{ color: 'var(--red)' }}>
                <XCircle size={12} /> Remove Item
              </button>
            )}
            {canWrite && (
              <button onClick={onSave} className="btn-ghost btn-xs" style={{ color: 'var(--green)' }}>
                <Save size={12} /> Save Items
              </button>
            )}
          </div>
        </div>
...
function WashCareInstructions() {
  const careItems = [
    {
      title: 'Wait Before Washing',
      detail: 'Wait at least 48-72 hours after installation before washing your vehicle to allow the vinyl to fully adhere.',
      icon: '⏱',
    },
    {
      title: 'Hand Wash Only',
      detail: 'Always hand wash your wrapped vehicle using a soft sponge or microfiber cloth with mild automotive soap. Avoid automatic car washes with brushes — they can scratch, lift, or damage the vinyl.',
      icon: '🧽',
    },
    {
      title: 'Use Mild Soap',
      detail: 'Use a gentle, non-abrasive automotive detergent. Avoid harsh chemicals, solvents, or degreasers that can damage the vinyl finish.',
      icon: '🧴',
    },
    {
      title: 'Rinse Thoroughly',
      detail: 'Rinse the vehicle with clean water before and after washing to remove loose dirt and prevent scratching.',
      icon: '💧',
    },
    {
      title: 'Dry Properly',
      detail: 'Use a clean microfiber towel or chamois to dry the wrap. Avoid letting water spots air-dry as they can leave mineral deposits.',
      icon: '🏁',
    },
    {
      title: 'Avoid High-Pressure Washing',
      detail: 'If using a pressure washer, keep it below 2,000 PSI, maintain at least 12 inches distance, and spray at a 45-degree angle. Never direct high pressure at edges or seams.',
      icon: '🚿',
    },
    {
      title: 'Spot Clean Promptly',
      detail: 'Remove bird droppings, tree sap, bug splatter, and fuel spills as soon as possible. Use isopropyl alcohol (IPA) or a vinyl-safe cleaner for stubborn spots.',
      icon: '🎯',
    },
    {
      title: 'Protect From Sun Exposure',
      detail: 'Park in a garage or shaded area when possible. Prolonged UV exposure can cause fading and degradation over time. Consider using a car cover for long-term outdoor storage.',
      icon: '☀️',
    },
    {
      title: 'No Wax or Polish on Matte/Satin',
      detail: 'Do not apply wax, polish, or any product designed for glossy paint on matte or satin finishes — it will create unwanted shine. Use only matte-specific care products.',
      icon: '✨',
    },
    {
      title: 'Gloss Wraps Can Be Waxed',
      detail: 'Gloss vinyl wraps can benefit from a vinyl-safe wax or sealant to maintain shine and add a layer of protection.',
      icon: '💎',
    },
  ]
  ...
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 16, fontFamily: 'Barlow Condensed, sans-serif' }}>
        Vinyl Wrap Wash & Care Instructions
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {careItems.map((item, idx) => (
          <div key={idx} style={{
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text1)', marginBottom: 2 }}>{item.title}</div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', marginBottom: 4 }}>Warranty Note</div>
        <div style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.5 }}>
          Proper care and maintenance are essential to keep your wrap looking its best and to maintain warranty coverage.
          Damage caused by improper washing, harsh chemicals, or neglect may void the warranty.
          Contact USA Wrap Co with any questions about caring for your wrap.
        </div>
      </div>
    </div>
  )
...
const INV_SURFACE_ISSUES = ['Rust spots', 'Deep scratches', 'Dents', 'Previous vinyl', 'Fading', 'Clear coat issues']
const INV_PREP_TIMES = ['None', '1hr', '2hr', '4hr', 'Custom']

function InvSurveyTab({
  ...
  const [vehicleEntry, setVehicleEntry] = useState<InvVehicleEntry | null>(null)
  ...
  useEffect(() => { invFetchAllMakes().then(setAllMakes) }, [])

  useEffect(() => {
    if (surveyData.vehicleMake) {
      invFetchModelsForMake(surveyData.vehicleMake, surveyData.vehicleYear).then(ms =>
  ...
  useEffect(() => {
    if (surveyData.vehicleMake && surveyData.vehicleModel) {
      invLookupVehicle(surveyData.vehicleMake, surveyData.vehicleModel, surveyData.vehicleYear)
  ...
  function selectMake(make: string) {
    ...
    invFetchModelsForMake(make, surveyData.vehicleYear).then(ms => setModelInfos(ms.map(m => ({ model: m.model, sqft: m.sqft }))))
  }

  async function selectModel(model: string) {
    setModelOpen(false); setModelFilter('')
    onFieldChange('vehicleModel', model)
    const v = await invLookupVehicle(surveyData.vehicleMake, model, surveyData.vehicleYear)
    setVehicleEntry(v)
  ...
  function getWrapSqft(v: InvVehicleEntry | null, scope: string): number {
  ...
  const sLbl: React.CSSProperties = { display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4, fontFamily: invHeadingFont }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700, fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text1)' }}>
      ...
      {/* ── VEHICLE INFO ─────────────────────────────────────────────────────── */}
      <div style={sCard}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={ddStyle}>
                {filteredModels.map(m => (
                  <div key={m.model} style={{ ...optSt, display: 'flex', justifyContent: 'space-between' }}
                    ...
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <span>{m.model}</span>
                    {m.sqft && <span style={{ fontSize: 10, color: 'var(--text3)', fontFamily: invMonoFont }}>{m.sqft}sqft</span>}
                  </div>
                ))}
              </div>
            )}
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['Front', 'Driver Side', 'Passenger Side', 'Rear', 'Damage'].map(angle => (
              <label key={angle} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '10px 14px', borderRadius: 8, border: '1px dashed var(--surface2)', cursor: canWrite ? 'pointer' : 'default', background: 'var(--bg)', minWidth: 80, color: photoAngles.includes(angle) ? 'var(--green)' : 'var(--text3)' }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, fontFamily: invHeadingFont }}>{angle}</span>
                {canWrite && (
          ...
          {photoAngles.length > 0 && <div style={{ fontSize: 11, color: 'var(--green)', marginTop: 6 }}>{photoAngles.length} angle(s): {photoAngles.join(', ')}</div>}
        </div>
      </div>

      {/* ── WRAP SCOPE ──────────────────────────────────────────────────────── */}
      <div style={sCard}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--purple)', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {sqftPreview !== null && vehicleEntry && (
          <div style={{ display: 'flex', gap: 20, marginBottom: 14, padding: '10px 14px', borderRadius: 8, background: 'rgba(34,192,122,0.06)', border: '1px solid rgba(34,192,122,0.15)' }}>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text3)', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Est. Sqft</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--green)', fontFamily: invMonoFont }}>{sqftPreview}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text3)', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Full Wrap Sqft</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)', fontFamily: invMonoFont }}>{vehicleEntry.sqft}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text3)', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Base Rate</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)', fontFamily: invMonoFont }}>${vehicleEntry.basePrice}</div>
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text3)', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Install Hrs</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)', fontFamily: invMonoFont }}>{vehicleEntry.installHours}h</div>
            </div>
          </div>
        )}
      ...
      {/* ── SURFACE CONDITION ────────────────────────────────────────────────── */}
      <div style={sCard}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--amber)', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {/* ── CUSTOMER SIGN-OFF ────────────────────────────────────────────────── */}
      <div style={sCard}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--green)', fontFamily: invHeadingFont, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
