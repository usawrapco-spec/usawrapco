    ...
    async function load() {
      ...
      if (data && data.length > 0) {
        const loaded: LineItemState[] = data.map(row => ({
          ...
          marWrapType: row.mar_wrap_type || 'printed',
          marTransom: row.mar_transom || false,
          marTransomWidth: row.mar_transom_width ? Number(row.mar_transom_width) : undefined,
          marTransomHeight: row.mar_transom_height ? Number(row.mar_transom_height) : undefined,
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const rows = itemsWithCalc.map((item, i) => ({
        ...
        mar_wrap_type: item.marWrapType || 'printed',
        mar_transom: item.marTransom || false,
        mar_transom_width: item.marTransomWidth || null,
        mar_transom_height: item.marTransomHeight || null,
