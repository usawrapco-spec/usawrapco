  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Totals summary below line items in items tab */}
          {activeTab === 'items' && lineItemsList.length > 0 && (
            <div className="card">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
                <div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4, fontFamily: headingFont }}>Payment Terms</label>
                      <select value={paymentTerms} onChange={e => setPaymentTerms(e.target.value)} className="field" disabled={!canWrite}>
                ...
                <div style={{ padding: '0 0 0 16px', borderLeft: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                    ...
                    {discount > 0 && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <span style={{ fontFamily: monoFont, fontVariantNumeric: 'tabular-nums', fontSize: 14, color: 'var(--green)', fontWeight: 600 }}>-{fmtCurrency(discount)}</span>
                      </div>
                    )}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {canWrite && (
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => handleCreateJob('per_item')} className="btn-ghost btn-sm">
                <Package size={13} style={{ color: 'var(--cyan)' }} /> Job Per Item
              </button>
              <button onClick={handleConvertToInvoice} className="btn-ghost btn-sm">
                <FileText size={13} style={{ color: 'var(--purple, #8b5cf6)' }} /> To Invoice
              </button>
              {status !== 'in_progress' && (
                <button onClick={() => handleStatusChange('in_progress')} className="btn-ghost btn-sm">
                  <ArrowRight size={13} style={{ color: 'var(--accent)' }} /> In Progress
                </button>
              )}
              {status !== 'completed' && (
                <button onClick={() => handleStatusChange('completed')} className="btn-ghost btn-sm">
                  <CheckCircle2 size={13} style={{ color: 'var(--green)' }} /> Complete
                </button>
              )}
              {status !== 'on_hold' && (
                <button onClick={() => handleStatusChange('on_hold')} className="btn-ghost btn-sm">
                  <AlertTriangle size={13} style={{ color: 'var(--amber)' }} /> On Hold
                </button>
              )}
            </div>
          )}
